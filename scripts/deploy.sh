...
gcloud run deploy "$SERVICE" --project "$GOOGLE_CLOUD_PROJECT" --region "$REGION" \
  ...
  --set-env-vars "GOOGLE_CLOUD_PROJECT=${GOOGLE_CLOUD_PROJECT},GEMINI_MODEL=${GEMINI_MODEL:-gemini-2.5-flash},GMI_MODEL=${GMI_MODEL:-meta-llama/Llama-3.3-70B-Instruct}" \
